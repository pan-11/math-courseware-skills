...
当前源码的整课制作顺序：资料分析→情境与故事/数学定稿→**整课必做视频创作与准备（内含风格和实际资产制作）**→**含视频预留页的逐页文案**→页面图片→可编辑PPT与三文稿→成片回填和课堂播放检查。从头使用整套Skill制作时，情景视频必做，至少一段；不再默认可选。仅校正已有课件等局部任务按实际范围处理。

当前源码已接入2026-09-18的六步视频流程与十二Skill，以及对应的整课交接与可编辑制作规则。最近发行包仍是0.2.1（七模块），不包含这些后续源码修订；使用新流程请读取当前仓库源码。真实课件、服务商出图、视频效果、可画往返和WPS显示需要首课实测；测试夹具不能代替首课验收。
...
选择情境和风格，成组确认故事/数学、整课视频方案与制作流程、共享资产、页面内容、带字图片试样及整套图片；可画导入、逐页拆层、导出PPTX由你完成。Codex核对真实回传后直接整套回填文字，完成后统一逐页检查；不设置单页文字试样或坐标采用前置。视频小样及成片按实际结果审阅，最终在WPS检查播放并设置动画。已确认的同版内容不重复确认；文稿不增加最终审批。
