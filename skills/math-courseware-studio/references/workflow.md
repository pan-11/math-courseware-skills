...
十二个Skill共用这条规则，单独调用专业模块也生效。每条用户可见回复，包括首次说明、执行中进度、回答追问、缺项提问、等待回传和最终交付，都在正常称呼后先写以下两项，再写正文；不能只在首次启动时说明。视频最终操作包是开头顺序的特例：先按[统一交付回复](../../math-courseware-video-prompts/references/delivery-reply.md)说“这是第几个视频，现在可以开始制作视频了”（缺项时如实写准备状态），随即保留下面两项及正文操作表；运镜与固定讲话都适用。

```text
当前步骤：<整课阶段或独立模块> → <正在执行的具体子步骤/对象>（<实际状态>）。
本步最终产物：<这一步完成时应交付的具体内容或文件>。
```

沿用用户的称呼偏好，不把示例姓名固定到其他用户。两项通常各一句，用中文说明工作，不只给Skill名或内部命令。

- **步骤取自真实进度。** 先根据已核对的本次范围、交接和有效成果定位。整课使用下表C1—C8名称；细分到当前动作，必要时注明V ID、页码或视频内部第几步。独立任务写“独立可编辑—文字回填”等，不套整课阶段或要求补跑上游。尚未读交接时写“恢复进度—核对已有成果”，本步产物为进度与缺项清单，不能猜已经到了哪个阶段。
- **产物按当前子步骤和授权填写。** 从本文件产物表及对应专业规则提取具体名称；只写本步要交付什么，不罗列全课未来成品。只讨论情境时列候选方案，不承诺实图；只排逐页内容时列逐页稿，不提前列图片PPT。已核实数量才写，文件尚未生成时不提供虚构路径或链接。
- **预期与完成分开。** “本步最终产物”说明本步目标，不证明文件已存在。正文交代已完成、仍在制作或缺失的部分；状态用进行中、待选择、等待回传、待核查、本步已完成等实际措辞。步骤完成后的交付仍标本步已完成，下一步另述，实际开始后再换步骤。模块完成、素材准备完成不代表整课或视频成片完成。
- **等待时仍对应当前交接。** 例如C6等待可画回传，本步产物写“用户回传的分层PPTX及收到后的页序/图层接收核查”，已经交出的去字PPT在正文说明；文字回填成品属于下一步，不提前列为这个等待子步骤的产物。未知回传质量仍标待检查。
- **保持现有路线和并行关系。** 整课视频写清“C3视频准备 → V001运镜第3步：导演分镜”，避免把两层编号混淆；固定讲话写“首帧与完整视频提示词”，不拆成六步或新增确认等待。C6等待可画、C7正在写稿时标当前C7，正文保留C6等待状态；同条回复确有两个并行重点时可并列步骤与对应产物，不把等待当完成。
- **说明放在聊天回复外层。** 不写入供复制的生图/视频提示词正文、逐字稿、PPT页面文案或JSON数据。使用说明、Skill维护等非课程执行问题按真实任务标注，不虚构C阶段。此说明不增加审批节点、不改变原有输入要求，也不因用户只追问进度就自动推进下一步。

例如，独立无字分层PPT回填开始时：

```text
当前步骤：独立可编辑 → B路线整套文字回填（进行中）。
本步最终产物：保留原图层的文字回填PPTX，以及逐页文字、位置和对象检查记录。
```

固定讲话素材交付时：

```text
当前步骤：独立视频 → 固定讲话首帧与完整提示词（本步已完成）。
本步最终产物：实际彩色首帧、一段含全部台词和表演要求的完整视频提示词，以及必要道具素材所在的普通文件夹。
```

后一示例仅能在本步图片和提示词确已交付时使用；实际视频尚未生成时，正文仍须保留这一事实。
...
| C7 配套 | 教师逐字稿、说课稿、教学设计、学生课堂学习单及电子黑板贴，与采用课件及视频衔接一致 | 图片课件锁定后与C6并行；内容/页序变化同步受影响文稿、学习单与素材，学习单/黑板贴仅在用户明确省略时免做 |
| C8 汇合 | 实际必做成片、实际PPT播放区域与回填/播放方式、WPS检查、最终文稿、学习单、黑板贴和交付清单 | 真正完成才声明整课完成；视频待到可交静态阶段稿 |

整课视频内部仍走剧本/25格预览→实际图片资产→导演双表→正式黑白板→固定风格→完整视频词。封面候选和共享风格/资产在视频第二步由plan与video-assets协同；已有采用资产复用。讲话简路由AI连续准备台词、实际彩色画面和完整视频提示词，同轮直接交首帧图片＋完整词；画面先展示供看效果，内部模块切换不再等待逐步采用，不生成不适用的导演表或黑白板。
...
| video-assets | 当前剧本与内部核查；运镜新制作还需真实25格预览及既定采用；固定讲话不需要25格或中途采用 |
| video-director / video-board | 前者核剧本/声音/实际资产，后者核导演及实际资产 |
| video-style / video-prompts | 运镜核实际采用彩色参考/正式板；固定讲话核实际首帧、完整台词及内部核查，组装完整词无需中途采用；均引用当前版本 |
...
固定讲话的`video-assets`、`video-prompts`与`video-upload`不再要求script/first-frame等材料的中途采用，仍检查真实文件、可解码图片、来源版本及内部核查；已拒绝版本不得复用。此例外只用于讲话材料准备，不自动登记用户采用、不替代整课蓝图/C3采用或成片播放验收。
...
- **学习单配套**：整课默认交[学生课堂学习单](../../math-courseware-documents/references/student-worksheet.md)，独立任务按指定范围。保存来源页/任务对应、实际DOCX、内容及分页检查、交付manifest；C8另核真实文件与同文件夹副本。现有export-documents仍只导出三份教师文稿，没有worksheet步骤/命令；控制器documents或complete通过不代表学习单已完成，不能只登记源稿。
- **黑板贴配套**：按[专门规范](../../math-courseware-documents/references/blackboard-stickers.md)保存独立spec/content/layout/manifest及实际视觉核查。整课默认必交，单模块按指定范围。现有控制器没有blackboard步骤，也不会由collect/complete自动核查；C8须另查真实文件、来源、PPT独立对象及显示、中文普通目录副本哈希。不能用三文稿通过代替，也不能把单做黑板贴冒充三文稿完成。
