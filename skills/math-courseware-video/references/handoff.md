...
运镜切镜路线的两个主要创作审阅为剧本＋预览、导演＋正式板。资产/风格采用可并入对应组，已确认的同版成果直接复用；技术细节由AI检查。内容实质改变依真实授权处理，不能借“减少询问”伪造采用。固定讲话由AI连续完成台词、单张彩色画面和完整视频提示词，同轮先展示实际图片，再给完整词供整体反馈，不增设中途确认。画面兼作预览与首帧，合格已有图直接复用；未收到采用不阻断材料交付，也不写为已采用。
...
讲话对外只交实际单首帧＋一段含全部台词的完整视频提示词，必要道具图同目录；格式统一见[讲话直接交付规范](../../math-courseware-video-prompts/references/talking-packet.md)。内部保留必要来源和核查，已有文档可留存，不新建整套多步骤说明要求用户确认。暂缺成果在start-here列清，不用空PNG或文本文件假冒图片。

交付前查看新增图、检查每个实际文件可打开、复制哈希相同、路径/镜号/版本相互对应；按本轮范围检查齐备性。起始说明直接告诉用户上传哪些实际文件、复制哪段词、哪些参数仍需配置。

最终操作包的聊天外层统一使用[视频最后一步回复](../../math-courseware-video-prompts/references/delivery-reply.md)，先说第几个视频和是否可以开始制作，再展示两张操作表及实际链接。运镜每次生成有独立完整TXT，固定讲话沿用talking-prompt.txt并保留首帧和完整正文；不能只给一个文件夹让用户自行查找对应关系。
