...
已有视频时读取`planning/video-handoff.md`及实际采用的[视频回填记录](../math-courseware-video/references/handoff.md)，把稳定video_id写入对应页的video_ids；按真实结束画面、台词和教师接话安排后续任务。未成片可按已定用途规划，不编造时长或画面；页序确定后补视频关联页码。文件关联不代表已自动嵌入视频，实际插入与课堂播放检查按现有工具或用户操作另记。
