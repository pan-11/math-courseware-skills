...
2026-09-19更新：整课/独立模块流程检查及可见原课分析已接入。两份安装提示词增加workflow规则、执行器和分析模板的实际文件核查；旧20260918完整包不含此次修订，应获取当前仓库完整源码。朋友通用安装与使用说明另有MD及离线HTML，用户授权同步现有GitHub仓库；其他电脑或项目中的安装副本仍需按实际版本更新。

2026-09-18更新（历史）：源码已按批准方案拆为十二个Skill，视频由总入口和writer/assets/director/storyboard/prompts五专业模块组成。两份安装提示词核对全部十二目录与相对引用，旧0.2.1 ZIP不能替代。用户当时另行授权同步到私有仓库；同步源码不代表其他项目安装副本自动更新，安装时仍需核对来源和manifest。下文版本记录保留历史含义。
