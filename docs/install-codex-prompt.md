版本要求：本提示词面向含视频模块的0.2.0或更高兼容版本。先核对实际来源包含math-courseware-video；若远端仍是六模块旧版，报告版本缺口并改用用户提供的完整0.2.0 ZIP，不声称已安装视频功能。
...
- 七个 Skill 放在 R/.agents/skills/<skill-name>/SKILL.md。核对当前宿主能否识别；当前会话无法确认时，记录“待刷新/新会话确认”，不要声称已出现在列表。
